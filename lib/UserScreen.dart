...
import 'package:caress/Predictiond.dart';
...
HealthValue? _bloodglucose;
HealthValue? _aiheart_rate;
HealthValue? _caloriesburned;
HealthValue? _weight;
...
class _UserDataState extends State<UserData> {
  ...
  Future func() async {
    ...
    HealthValue? bloodglucose;
    HealthValue? aiheartrate;
    HealthValue? caloriesburned;
    HealthValue? weight;
    ...
    for (final data in healthData) {
      ...
      if (data.type == HealthDataType.BLOOD_GLUCOSE) {
        if (data.value != null) {
          bloodglucose = data.value;
        }
      }
      if (data.type == HealthDataType.HEART_RATE) {
        if (data.value != null) {
          aiheartrate = data.value;
        }
      }
      if (data.type == HealthDataType.ACTIVE_ENERGY_BURNED) {
        if (data.value != null) {
          caloriesburned = data.value;
        }
      }
      if (data.type == HealthDataType.WEIGHT) {
        if (data.value != null) {
          weight = data.value;
        }
      }
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    ...
                    children: [
                      DataValue(
                        data: "${_caloriesburned} calories",
                        dataIcon: Icons.thermostat,
                        dataName: "Calories burned",
                      ),
                      DataValue(
                        data: "${_bp_d}",
                        dataIcon: Icons.directions_walk,
                        dataName: "Sleep (today)",
                      ),
                    ],
                  ),
                  SizedBox(height: 30),
